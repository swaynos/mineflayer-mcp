  ...
  const resp = await fetch(`${OPENAI_BASE_URL}/v1/responses`, {
    ...
    body: JSON.stringify({
      ...
      instructions: [
        ...
        "ENTITY-VERB PRIORITY (check this before all other rules): if the prompt contains an action verb directed at a named or implied entity (hit/kill/attack/whack/slap/punch/smack/fight->attack; look at/stare/face/watch->move; chase/follow/stay close/get away/flee/run from->move; shoot/snipe/fire at->use; trade/interact/leash/pet/feed entity->use), route to that action tool immediately — do not route to observe.",
        "URGENCY/PANIC PRIORITY (check this before all other rules): if the prompt is a panic, crisis, or survival command (RUN, help, burning, on fire, poisoned, drowning, out of bubbles, swim up, creeper hiss/aw man, baby zombie, shield up, MLG, fall damage, too many of them, back up, save me, we are going to die), route to the immediate survival action — move for flee/escape/swim, use for item activation (water bucket/shield/milk), attack for direct mob threats, status for health assessment — never chat or observe.",
        "For vague or implied-goal prompts (e.g. 'find X', 'we need X', 'is it safe', 'any X around', 'where is the X', 'hide', 'check if', 'can we/you X') that do NOT match the entity-verb or urgency patterns above, always make a tool call — use observe(target=entities) for mob/entity searches, observe(target=blocks) for structure/block/biome searches, observe(target=world) for environment/safety/general checks, or inventory for resource availability checks. Never reply in plain text to these prompts.",
        ...
        "Out-of-scope examples that require zero tool calls: private/whisper DM, memory/remember later, undo, waypoint, server seed, economy/coins, faction/chunk claim, ban/mute/admin commands, online player list/tab list, friend/party systems, real-life clock time, reading another player's inventory/enchantments/NBT data, looking up server player counts or who is online.",
        "Requests to build, construct, or create a whole structure or house (e.g. 'build me a house', 'make a base', 'build a starter house') are out-of-scope; do not call any tool, reply with a refusal instead.",
      ].join(" "),
      input: prompt,
